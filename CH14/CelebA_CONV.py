...
#Pipeline of trasnformations for data augmentation
torch.manual_seed(1)
...
batch_size = 32
torch.manual_seed(1)
train_dl = DataLoader(celeba_train_dataset, batch_size=batch_size,
                      shuffle=True)
valid_dl = DataLoader(celeba_valid_dataset, batch_size=batch_size,
                      shuffle=False)
test_dl = DataLoader(celeba_test_dataset, batch_size=batch_size,
                     shuffle=False)
...
model.add_module(
    'conv3',
    nn.Conv2d(in_channels=64, out_channels=124,
              kernel_size=3, padding=1)
)
...
model.add_module(
    'conv4',
    nn.Conv2d(in_channels=124, out_channels=256,
              kernel_size=3, padding=1)
)
model.add_module('relu4', nn.ReLU())

x = torch.ones([4,3,64,64])
print('Before adding global pooling: output = ',model(x).shape)

model.add_module('pool4', nn.AvgPool2d(kernel_size=8))
model.add_module('flatten', nn.Flatten())

x = torch.ones([4,3,64,64])
print('After global pooling and nn.Flatten(): output = ',model(x).shape)

model.add_module('fc or MLP', nn.Linear(256,1))
model.add_module('sigmoid', nn.Sigmoid())

x = torch.ones([4,3,64,64])
print('After fully connected layer (256x1) and sigmoid: output = ',model(x).shape)
